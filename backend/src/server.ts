...
export class Server {
  ...
  public start (): void {
    this.httpServer.listen( this.port, () => console.log( `Backend-App is listing on port ${this.port}!` ) );
  }
  ...
  private configureSockets (): void {
    ...
    new SocketConnector( io, [
      ( socket: Socket ) => console.log( '>> new socket connection', socket ), ChatSocketController.setListener
    ] );
  }
}
